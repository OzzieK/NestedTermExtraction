...
def model_save(model, path, stype='cls'): # cls ==> classification rnk ==> ranking
    model_name = model.__str__()
    if not os.path.exists(path):
        os.mkdir(path)

    filename = os.path.join(path, model_name)+'_'+stype
    torch.save(model.state_dict(), filename)


def model_load(model, path, stype='cls'): # cls ==> classification rnk ==> ranking
    model_name = model.__str__()
    filename = os.path.join(path, model_name)+'_'+stype
...
def evaluate(model, data, devData, rankingflag=False):
    ...
    for batch_idx in range(dev_batchs):
        ...
        if rankingflag:
            loss, TP, TPFP, TPFN, cTPFN = model.forward(batch_words_seq, batch_pos_seq, batch_seq_lengths, batch_char_seq,
                                                     batch_char_seq_lengths, batch_char_seq_recover, batch_labels,
                                                     batch_mask,
                                                     batch_texts, training=False, rankingflag=True)
            total_golden.append(cTPFN)
        else:
            loss, TP, TPFP, TPFN, *_ = model.forward(batch_words_seq, batch_pos_seq, batch_seq_lengths, batch_char_seq,
                                                batch_char_seq_lengths, batch_char_seq_recover, batch_labels, batch_mask,
    ...
    return avg_loss, precision, recall, f1, trecall, tf1
...
def training(model, data, trainData, devData):
        ...
        for batch_idx in range(train_batchs):
            ...
            if (batch_idx + 1) % data.evaluate_every == 0 and batch_idx > 0:
                ...
                if davg_loss < best_dev_loss:
                    ...
                    early_stop_count = 0
                    model_save(model.classifier, data.model_save_dir)
    ...
    model.classifier = model_load(model.classifier, data.model_save_dir)
    ...
    for _ridx in range(data.HP_epoch):
        ...
        for rbatch_idx in range(rtrain_batchs):
            ...
            loss, rTP, rTPFP, rTPFN, cTPFN = model.forward(batch_words_seq, batch_pos_seq, batch_seq_lengths, batch_char_seq,
                                                    batch_char_seq_lengths, batch_char_seq_recover, batch_labels,
            ...
            if (rbatch_idx + 1) % data.print_every == 0 and rbatch_idx > 0:
                print(
                    colored("Ranking model results at Epoch {} Batch {}: "
                           "#Average Loss: {:0.4f}  "
                           "#Average Prec: {:0.3f}  "
                           "#Average Recl: {:0.3f}  "
                           "#Average F1-V: {:0.3f}  "
                           "#Actual Recall: {:0.3f}  "
                           "#Actual F1: {:0.3f}".format(_ridx+1,  rbatch_idx+1,
                                                        ravg_loss,
                                                        ravg_precision,
                                                        ravg_recall,
                                                        ravg_f1,
            ...
            if (rbatch_idx + 1) % data.evaluate_every == 0 and rbatch_idx > 0:
                davg_loss, dprecision, drecall, df1, dtrecall, dtf1 = evaluate(model, data, devData, rankingflag=True)
                # print('Evaluating ...', end='\r')
                print(
                    colored("DEV results: #Average Loss: {:0.4f}  "
                           "#Average Prec: {:0.3f}  "
                           "#Average Recl: {:0.3f}  "
                           "#Average F1-V: {:0.3f}  "
                           "#Actual AVG Recl: {:0.3f}  "
                           "#Actual AVG F1: {:0.3f}".format(davg_loss,
                                                            dprecision,
                                                            drecall,
                                                            df1,
                                                            dtrecall,
                ...
                if davg_loss < rbest_dev_loss:
                    ...
                    rbest_dev_loc = [_ridx+1, rbatch_idx+1, davg_loss, dprecision, drecall, df1, dtrecall, dtf1]
                    rearly_stop_count = 0
                    model_save(model, data.model_save_dir, 'rnk')
                    ...
                    if rearly_stop_count >= data.earlystop:
                        ...
                        print('Best Dev Result Occurred at Epoch {} Batch {}'.format(rbest_dev_loc[0],
                                                                                          rbest_dev_loc[1]))
                        break
        if rearly_stop_count >= data.earlystop:
            break
...
def test(data, model, testData):
    ...
    model = model_load(model, data.model_save_dir, stype='rnk')
    model.eval()
    batch_size = data.HP_batch_size
    ...
    for batch_idx in range(test_batchs):
        ...
        loss, TP, TPFP, TPFN, cTPFN, cpredict, rpredict = model.forward(batch_words_seq, batch_pos_seq,
                                                                        batch_seq_lengths, batch_char_seq,
                                                                        batch_char_seq_lengths, batch_char_seq_recover,
                                                                        batch_labels, batch_mask, batch_texts,
                                                                        training=False, rankingflag=True, dump_result=True)
...
if __name__ == '__main__':
    ...
    data.HP_dropout = args.dropout
    data.model_save_dir = args.savemodel
    data.term_span = args.max_length
    ...
    print('\t\tLoss\tPrec\tRec\tF1\tTRec\tTF1')
    if args.status == 'train':
        print('Dev:\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}'.format(*dev_best_result[2:]))
    print('Test:\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}'.format(*test_result))

    summary_filename = './logs/'+'Dropout_{}_lr_{}_MaxLen_{}_HD_{}.txt'.format(data.HP_dropout, data.HP_lr, data.term_span, data.HP_hidden_dim)
    with open(summary_filename) as filout:
        ...
        filout.write('\tSeed:\t{}\n'.format(seed_num))
        filout.write('\nDev & Test Results\n')
        filout.write('\t\tLoss\tPrec\tRec\tF1\tTRec\tTF1\n')
        if args.status == 'train':
            filout.write('Dev:\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\n'.format(*dev_best_result[2:]))
        filout.write('Test:\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\t{:0.4f}\n'.format(*test_result))
